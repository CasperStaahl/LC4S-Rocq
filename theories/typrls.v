...
Variable d d' : Order.disp_t.
Variable P : finTBLatticeType d.
Variable Q : finTBLatticeType d'.
Variable fg : Lagois.type P Q.

Inductive CommSType d {A : finTBLatticeType d} {Term0 : SubSemantic.type Val} :
    (Store A) -> command Term0 -> A -> Prop :=
  | Tt (λ : Store A) t l:
...
Inductive PhraseSType :
    (Store P * Store Q) -> phrase Term Term' -> (P * Q) -> Prop :=
...
Lemma CommsSType_sound
    d0 (A : finTBLatticeType d0)
    {Term0 : SubSemantic.type Val}
    (λ : Store A) τ τ_0 (μ ν μ' ν' : Store Val)
    (c : command Term0) :
  CommSType λ c τ_0 ->
  μ ==[ c ]==> μ' ->
  ν ==[ c ]==> ν' ->
...
Proof.
  ...
  have λw''_le_λw'_impl_μup_eq_νup :
      forall w' w'', λ w'' <= λ w' ->
          [eta μ with w' |-> μ w''] w = [eta ν with w' |-> ν w''] w.
    ...
    move/eqP: w_eq_expx => <-.
    exact: λw_le_τ.
  inversion μ_c_μ'; subst; inversion ν_c_ν'; subst;
      inversion λ_gives_c_type_τ'; subst; first last.
  ...
  - have w_notassin_t_impl_μ'w_eq_ν'w : ~~(assigned t w) -> μ' w = ν' w.
    ...
    case: w λw_le_τ w_notassin_t_impl_μ'w_eq_ν'w {λw''_le_λw'_impl_μup_eq_νup}
        => n λn_le_τ w_notassin_t_impl_μ'w_eq_ν'w; first last.
...
Lemma T_sound
    d0 (A : finTBLatticeType d0)
    d0' (B : finTBLatticeType d0')
    (hi : Lagois.type A B)
    (λ : Store A) (λ' : Store B)
    (m' : B) (y x' : ident)
    (w : Var) (μ ν μ' ν' : Store Val) :
  m' = hi.1 (hi.2 m') ->
  hi.2 (λ' (exp x')) <= λ (imp y) ->
  λ w <= hi.2 m' ->
  (forall w' : Var, λ' w' <= m' -> μ' w' =  ν' w') ->
  (forall w : Var, λ w <= hi.2 m' -> μ w = ν w) ->
  (if w == imp y then μ' (exp x') else μ w) =
      (if w == imp y then ν' (exp x') else ν w).
Proof.
  move=> m'_eq_him' iλ'x'_le_λy λw_le_im' μ'w'_eq_ν'w' μw_eq_νw.
  case w_eq_y: (w == imp y); last first.
    exact: μw_eq_νw λw_le_im'.
  apply: μ'w'_eq_ν'w'.
  have λ'x'_le_hiλ'x' : λ' (exp x') <= hi.1 (hi.2 (λ' (exp x')))
    by exact: lc2.
  have iλ'x'_le_im' : hi.2 (λ' (exp x')) <= hi.2 m'.
    move/eqP in w_eq_y; rewrite -w_eq_y in iλ'x'_le_λy.
    exact: le_trans iλ'x'_le_λy λw_le_im'.
  have hiλ'x'_le_m' : hi.1 (hi.2 (λ' (exp x'))) <= m'.
    move /(omorph_le hi.1) in iλ'x'_le_im'.
    by rewrite -m'_eq_him' in  iλ'x'_le_im'.
...
Proof.
  ...
  move=> s_1 p {}μ μ_1 {}μ_f μ_s1_μ1 IH μ1_p_μf ν ν_f t_0 λ
         ν_s1p_νf λ_gives_s1p_type_t0.
  case E: _ _ / ν_s1p_νf => [|s_1' p' {}ν ν_1 {}ν_f ν_s1_ν1 ν1_p_νf]//.
  move: E => [s1_eq_s1' p_eq_p'].
  rewrite -{s_1'}s1_eq_s1' in ν_s1_ν1; rewrite -{p'}p_eq_p' in ν1_p_νf.
  case E: _ _ / λ_gives_s1p_type_t0 =>
      [|{}λ t_p t_s p' s_1' λ_gives_p_tp λ_gives_s1_ts]//.
  move: E => [s1_eq_s1' p_eq_p']; rewrite -{s_1'}s1_eq_s1' in λ_gives_s1_ts;
      rewrite -{p'}p_eq_p' in λ_gives_p_tp.
  move=> μ1w_eq_ν1w μ2w_eq_ν2w.
  move/(_ ν ν_1 t_s λ ν_s1_ν1 λ_gives_s1_ts μ1w_eq_ν1w μ2w_eq_ν2w): IH =>
      {μ1w_eq_ν1w μ2w_eq_ν2w}[μ11w_eq_ν11w μ12w_eq_ν12w].
  case: p μ1_p_μf ν1_p_νf λ_gives_p_tp =>
      [ c | c' | y x' | y' x ] μ1_p_μf ν1_p_νf λ_gives_p_tp.
  ...
  - inversion μ1_p_μf; inversion ν1_p_νf; inversion λ_gives_p_tp; subst => /=.
    split=> [w λ0w_le_l | // ].
    exact: T_sound m'_eq_fl H11 λ0w_le_l μ12w_eq_ν12w μ11w_eq_ν11w.
  - inversion μ1_p_μf; inversion ν1_p_νf; inversion λ_gives_p_tp.
    move: l_eq_gm' m'_eq_fl. subst => l_eq_gm' m'_eq_fl /=.
    split=> [ // | w' λ'w_le_m'].
    rewrite m'_eq_fl in l_eq_gm' λ'w_le_m' μ12w_eq_ν12w.
    have l_eq_gm'_ : l = (fg.2, fg.1).1 ((fg.2, fg.1).2 l) by exact: l_eq_gm'.
    exact (T_sound l_eq_gm'_ H11 λ'w_le_m' μ11w_eq_ν11w μ12w_eq_ν12w).
Qed.
